...
- **База данных** хранится в Docker volume `postgres-data`.
- **Загруженные изображения** (через админку) хранятся в Docker volume `ramo-uploads`.

Оба volume сохраняются между перезапусками контейнера, но **теряются**, если их удалить.
...
- картинки меню из `public/images/menu/` (встроены в образ).
